...
public class AvailableFilter implements Filter
{
                          ...
                          FilterChain chain) throws IOException, ServletException
    {
        ...
        System.out.println("Session user_id = " + req.getSession().getAttribute("id") +
        ...
        if (!isFreeRoute(currentUrl)) {
            if (req.getSession().getAttribute("id") == null) {
                ...
                resp.sendRedirect("/todolist/login?message=this page available only to authorized users");
                return;
            } else {
        ...
        if (isOnlyNotAuth(currentUrl) && req.getSession().getAttribute("userId") != null){
            resp.sendRedirect("/todolist/error?message=for this page log out");
            return;
        }
